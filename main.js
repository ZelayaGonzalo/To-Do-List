...
function showSelectedTasks(name) {
  ...
  const arrayToShow = allTasks.filter((task) => task.project === name);
...
form.addEventListener('submit', (event) => {
  ...
  const task = createTask(
    ...
    new Date(form.elements.date.value).toDateString(),
    false
  );
...
projectForm.addEventListener('submit', (event) => {
  ...
  const project = createProject(
    projectForm.elements.title.value,
...
function ShowSelectedbyDate(id) {
  ...
  if (id === 'Today') {
    ...
    const show = allTasks.filter(
      (task) => new Date(task.dueDate).getMonth() == today.getMonth()
    );
